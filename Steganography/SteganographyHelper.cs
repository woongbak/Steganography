...
namespace Steganography
{
    class SteganographyHelper
    {
        ...
        public static Bitmap embedText(string text, Bitmap bmp) // 텍스트와 이미지를 입력받아 실행하는 함수 선언
        {
            ...
            for (int i = 0; i < bmp.Height; i++) // 해당 파일의 높이만큼 실행하는 반복문
            {
                for (int j = 0; j < bmp.Width; j++) // 해당 파일의 너비만큼 실행하는 반복문
                {
                    Color pixel = bmp.GetPixel(j, i); // 픽셀의 색 값을 가져옴
                    ...
                    for (int n = 0; n < 3; n++) // 3번 실행해주는 반복문
                    {
                            ...
                            case 2:
                                {
                                    if (state == State.Hiding) // state가 0이라면 실행
                                    {
                                        B += charValue % 2; // 파랑 값에 숨겨질 문자열 값 % 2 값을 추가
                        ...
                        pixelElementIndex++; // pixelElementIndex에 1을 더해준다.
