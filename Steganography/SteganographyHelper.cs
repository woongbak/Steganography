...
namespace Steganography
{
    class SteganographyHelper
    {
        ...
        public static Bitmap embedText(string text, Bitmap bmp)
        {
            ...
            int R = 0, G = 0, B = 0; // R, G, B 값을 저장할 변수 선언

            // 이미지의 사이즈 만큼의 모든 픽셀을 이중 for문을 통해서 순회
            for (int i = 0; i < bmp.Height; i++)            // 이미지의 높이 만큼 반복문을 돌림
            {
                for (int j = 0; j < bmp.Width; j++)         // 이미지의 넓이 만큼 반복문을 돌림
                {
                    Color pixel = bmp.GetPixel(j, i);       // 각 위치에 있는 픽셀의 값을 가져옴

                    // pixel.R G B의 값을 정수형 R, G, B에 저장
                    // 마지막 값(LSB)을 0으로 맞춰줌 -> 도화지를 만든다고 생각
                    R = pixel.R - pixel.R % 2;
                    G = pixel.G - pixel.G % 2;
                    B = pixel.B - pixel.B % 2;

                    // 각 픽셀의 R, G, B값을 반복문으로 확인하여 조건문을 통해 그 값을 변경. 각 픽셀 당 3바이트
                    for (int n = 0; n < 3; n++)     
                    {
                        if (pixelElementIndex % 8 == 0) // 8비트 마다 끊어줌
                        {
                            if (state == State.Filling_With_Zeros && zeros == 8)    // 숨길것이 없는 상태 + 숨길것이 없는상태에서 8번째이면
                            {
                                ...
                                return bmp; // 위의 모든 조건이 만족하면 스테가노가 끝났기에 bmp값 반환
                            }
        ...
        public static string extractText(Bitmap bmp)       // 데이터를 추출하는 함수
        {
            ...
            for (int i = 0; i < bmp.Height; i++)    // 이미지의 높이 만큼 반복문을 돌림
            {
                for (int j = 0; j < bmp.Width; j++) // 이미지의 너비 만큼 반복문을 돌림
                {
                    Color pixel = bmp.GetPixel(j, i);   // 각 위치에 있는 픽셀의 값을 가져옴
